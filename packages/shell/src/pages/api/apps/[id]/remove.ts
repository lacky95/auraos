 ...
 *   2. `rm -rf /workspace/apps/<id>` — chokidar's `unlink` watcher emits
...
async function deleteApp(appId: string | undefined): Promise<Response> {
  ...
  // Path-escape guard: resolve(dest) must stay under APPS_DIR. If a regex
  // bypass slips through one day we still don't rm a sibling system dir.
  const resolved = resolve(dest);
  if (!resolved.startsWith(resolve(APPS_DIR) + '/')) {
    return errorResponse(`Refusing to delete outside ${APPS_DIR}: ${resolved}`, 400);
  }
